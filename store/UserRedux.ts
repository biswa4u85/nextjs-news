...
export const siteLogin: any = createAsyncThunk(
    'user/siteLogin',
    async (params, { rejectWithValue }) => {
        const response: any = await loginApi(`/api/method/${Config.frappe_custom_app}.authentication.login`, params)
...
export const signUpUser: any = createAsyncThunk(
    ...
    async (params: any, { rejectWithValue }) => {
        const response: any = await postCmdApi({ cmd: 'frappe.core.doctype.user.user.sign_up', redirect_to: ' ', ...params })
        if (response.status === 'error') {
            return rejectWithValue(response.data)
        }
...
export const counterSlice = createSlice({
    ...
    extraReducers: {
        ...
        [siteLogin.fulfilled]: (state: any, action) => {
            state.isFetching = false
            state.error = null
